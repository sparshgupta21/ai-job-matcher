"""
AI Job Matcher - LLM Inference Engine
Supports HF Inference API (serverless) for small open-source models.
"""
...
def call_llm(model_id, system_prompt, user_content, max_tokens=2048, temperature=0.3):
    """
    Call an LLM via the HuggingFace Inference API.
...
def analyze_cv_with_llm(model_id, cv_text):
    ...
    return call_llm(model_id, CV_ANALYSIS_PROMPT, safe_content)
...
def match_jobs_with_llm(model_id, cv_text, jobs_data):
    ...
    return call_llm(model_id, JOB_MATCH_PROMPT, user_content, max_tokens=3000)
...
def generate_search_queries(model_id, cv_text):
    ...
    safe_content = wrap_user_content_safely(cv_text)
    return call_llm(model_id, SEARCH_QUERY_PROMPT, safe_content, max_tokens=500)
...
def generate_autofill(model_id, cv_text):
    ...
    safe_content = wrap_user_content_safely(cv_text)
    return call_llm(model_id, AUTOFILL_PROMPT, safe_content, max_tokens=1000)
